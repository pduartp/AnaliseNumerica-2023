...
[x2, Iter, Erro] =  gauss_Seidel(n-1, A, b, 10e-3, 100)
...
disp("- Comparativo entre x (gaussIngenua), x2 (gauss_Seidel) e solReal (tabela)");
disp([x, x2, solReal]);

xData = linspace(1,9,9);
...
title("Comparativo entre Eliminação de Gauss Ingênua (azul), Gauss Seidel (vermelho) e solReal (verde)");
plot(xData, y1Data,'x-b');
plot(xData,y2Data,'x-r');
plot(xData,y3Data,'x-g');
